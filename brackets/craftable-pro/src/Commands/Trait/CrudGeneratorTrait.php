...
trait CrudGeneratorTrait
{
    ...
    protected ?Collection $tableColumns = null;
    ...
    protected ?Collection $indexColumns = null;

    protected ?Collection $formColumns = null;
    ...
    public function chooseTableName()
    {
        ...
        $tables = collect(Schema::getTableListing())->filter(function ($table) {
    ...
    protected function buildTableColumns()
    {
        ...
        $this->tableIndexes = collect(Schema::getIndexes($this->tableName));
        //        $this->tableForeignKeys = collect(Schema::getConnection()->getDoctrineSchemaManager()->listTableForeignKeys($this->tableName));
        $this->tableForeignKeys = collect(Schema::getForeignKeys($this->tableName));
    ...
    {
        return collect(Schema::getColumns($tableName))->map(function ($column) use ($tableName) {
            $columnType = Schema::getColumnType($tableName, $column['name']);
            ...
            return [
                ...
                'translatable' => false,
                'publishable' => false,
                'hidden' => in_array($column['name'], $this->hideColumns),
            ];
        })->filter();
    }
    ...
    {
        $typeScriptPath = $this->makeDirectory(resource_path("js/craftable-pro/Pages/{$this->className}/types.d.ts"));
    ...
    protected function buildDryRun()
    {
        ...
        $this->info(
            'php artisan craftable-pro:generate-crud '.$this->tableName.
    ...
    protected function appendLinkInSidebar()
    {
        $this->components->task('Sidebar', function () {
            ...
            $sidebar = resource_path('js/craftable-pro/Components/Sidebar.vue');
            $file = $this->files->get($sidebar);
            $append = '<SidebarItem
                        :href="route(\'craftable-pro.'.$link.'.index\')"
