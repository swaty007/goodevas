      ...
      <div class="flex gap-4">
        <Button variant="outline" color="gray" class="relative">
          <label
            ...
            @drop.prevent="onFilesDrop"
          >
            <span>{{ $t("craftable-pro", "Change") }}</span>
          </label>
          <input
            :id="name"
            :name="name"
            type="file"
            class="absolute inset-0 -z-10 h-full w-full cursor-pointer rounded-md border-gray-300 dark:border-gray-600 opacity-0"
            @input="onFilesSelect"
          />
        </Button>
        <Button
...
const uploadFile = (file: File) => {
  if (file.size > props.maxFileSize!) {
    toast.error(
      trans("craftable-pro", "File size is too big!")
