<template>
    <Listing :baseUrl="props.baseUrl" :data="products" dataKey="products">
        <template #actions>
            <div class="flex flex-wrap gap-1">
                <Tag color="danger" rounded size="sm">
                    {{ $t('global', 'Critical') }}
                </Tag>
                <Tag color="warning" rounded size="sm">
                    {{ $t('global', 'Shortage') }}
                </Tag>
                <Tag color="success" rounded size="sm">
                    {{ $t('global', 'Normal') }}
                </Tag>
                <Tag color="info" rounded size="sm">
                    {{ $t('global', 'Stocked') }}
                </Tag>
                <Tag color="purple" rounded size="sm">
                    {{ $t('global', 'Surplus') }}
                </Tag>
            </div>
                        ...
                        @click.prevent="
                            () => {
                                bulkAction(
                                    'post',
                                    route(
                                        'craftable-pro.products.bulk-destroy',
                                    ),
                                    {
                ...
                <template v-for="warehouse in warehousesWithDates">
                    <template v-if="!warehouse?.virtual">
                        <ListingHeaderCell
                            ...
                            v-for="(
                                date, index
                            ) of warehouse.futureIncomesDates"
                        ...
                        >
                            ...
                            <div class="flex gap-1">
                                ...
                                <ModalDateAdd
                                    v-if="
                                        index ===
                                        warehouse.futureIncomesDates.length - 1
                                    "
                                    :warehouse="warehouse"
                                    @submit="dateSubmit"
                                />
        ...
        <template #tableRow="{ item, action }: any">
            ...
            <ListingDataCell>
                <Avatar :src="item?.additional_data?.image" name="Logo" />
            </ListingDataCell>
            <ListingDataCell>
            ...
            <template v-else-if="forecast">
                <template v-for="warehouse in warehouses">
                    <ListingDataCell v-if="!warehouse?.virtual" class="">
                        <div class="tooltip relative flex items-center gap-2">
                            <div
                                :class="
                                    stockClass(
                                        item,
                            ...
                            >
                                {{
                                    (getConsumption(item, warehouse) /
                                        filtersForm.days) *
                                        getMinMaxRange(warehouse, 'success')
                                            .min -
                                    (getPivotValue(
                                        item,
                                        warehouse,
                            ...
                            <span
                                class="tooltip__text bg-gray-800 text-gray-100 dark:bg-gray-100 dark:text-gray-700"
                            >
                                {{ $t('global', 'Income') }}:
                                {{
                                    getIncomeByDateAndWarehouse(
...
const warehousesWithDates = computed(() => {
    return Object.values(props.warehouses).map((warehouse) => {
        const futureIncomesDates = Array.from(
            new Set([
                ...warehouse.futureIncomesDates,
                ...mergeDates.value
                    .filter((d) => d.warehouse_id === warehouse.id)
                    .map((d) => d.income_date),
            ]),
...
const updateProductIncome = debounce(
    (product: Product, warehouse: Warehouse, date: string, value: string) => {
        action(
            'patch',
            route('craftable-pro.products.update-income', {
                product: product.id,
...
const getIncomeByDateAndWarehouse = (
    product: Product,
    warehouse: Warehouse,
    date: string,
) => {
    return (
        product.incomes?.find(
            (income) =>
                income.income_date === date &&
                income.warehouse_id === warehouse.id,
        )?.quantity ?? 0
    ); // Возвращаем 0 по умолчанию, если данных нет
};

const getIncomeByWarehouse = (product: Product, warehouse: Warehouse) => {
    return (
        product.incomes
            ?.filter((income) => income.warehouse_id === warehouse.id)
            .reduce((acc, income) => acc + income.quantity, 0) ?? 0
    ); // Возвращаем 0 по умолчанию, если данных нет
};

const getConsumption = (product: Product, warehouse: Warehouse) => {
    return (
        product?.stock_changes?.total_consumption?.[warehouse.ysell_name] ?? 0
    );
};
