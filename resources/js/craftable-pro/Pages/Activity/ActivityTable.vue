        ...
        <template #tableRow="{ item, action }: any">
            ...
            <ListingDataCell>
                <template
                    v-if="
                        item.subject_type ===
                        'Brackets\\CraftablePro\\Models\\CraftableProUser'
                    "
                >
                    {{ $t('global', 'Admin') }}
                </template>
            ...
            <ListingDataCell>
                <div v-if="item.created_at" class="flex gap-2">
...
const { filtersForm, resetFilters, activeFiltersCount } = useListingFilters(
    ...
    {
        causer_type:
            (usePage().props as unknown as PageProps).filter?.causer_type ?? [],
        subject_type:
            (usePage().props as unknown as PageProps).filter?.subject_type ??
            [],
