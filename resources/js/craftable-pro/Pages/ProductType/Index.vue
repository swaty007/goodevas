        ...
        <Modal type="danger">
          ...
          <template #buttons="{ setIsOpen }">
            <Button
              @click.prevent="
                () => {
                  bulkAction('post', route('craftable-pro.product-types.bulk-destroy'), {
      ...
      <template #tableRow="{ item, action }: any">
        ...
        <ListingDataCell>
          <div class="flex items-center justify-end gap-3">
            <IconButton
              :as="Link"
              :href="route('craftable-pro.product-types.edit', item)"
              variant="ghost"
              color="gray"
            ...
            <Modal type="danger">
              ...
              <template #buttons="{ setIsOpen }">
                <Button
                  @click.prevent="
                    () => {
                      action('delete', route('craftable-pro.product-types.destroy', item), {
                        onFinish: () => setIsOpen(false),
                      });
                    }
                  "
...
const downloadFile = () => {
    ...
    if(url.length > 1) {
      window.location = route('craftable-pro.product-types.export', url.pop()).slice(0, -1);
    } else {
